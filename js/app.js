    ...
    (() => {
        ...
        if (document.querySelector("[data-datepicker]")) {
            const picker = datepicker_min("[data-datepicker]", {
                customDays: [ "Sun", "Mo", "Tu", "Wed", "Th", "Fr", "Sat" ],
                customMonths: [ "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" ],
                overlayButton: "Apply",
                overlayPlaceholder: "Year (4 number)",
                startDay: 1,
        ...
        document.addEventListener("click", (e => {
            ...
            OpenSidebar(targetElement);
            if (targetElement.closest(".checkbox") && targetElement.closest(".filter-catalog__body") && targetElement.tagName != "INPUT") showFilterClear(targetElement); else if (targetElement.closest(".filter-catalog__clear")) clearFilter(targetElement); else if (targetElement.closest(".account-dashboard__slide-button")) sidebarButtonFunc();
        }));
        ...
        function showFilterClear(element) {
            const filterBody = element.closest(".filter-catalog");
            const clearButton = filterBody.querySelector(".filter-catalog__clear");
            setTimeout((() => {
                const checkedElements = filterBody.querySelectorAll(".checkbox__input:checked");
                checkedElements.length ? clearButton.classList.add("_active") : clearButton.classList.remove("_active");
            }), 0);
        }
        function clearFilter(element) {
            const clearBlock = element.closest(".filter-catalog");
            const inputFields = clearBlock.querySelectorAll(".price-filter__input");
            if (inputFields.length) inputFields.forEach((inputField => {
                inputField.value = "";
            }));
            const checkboxElements = clearBlock.querySelectorAll(".checkbox__input:checked");
        ...
        function OpenSidebar(element) {
            if (htmlElement.classList.contains("account-sidebar-open")) if (element.closest(".account-dashboard")) htmlElement.classList.add("account-sidebar-open"); else htmlElement.classList.remove("account-sidebar-open");
        }
        function sidebarButtonFunc() {
            if (!htmlElement.classList.contains("account-sidebar-open")) htmlElement.classList.add("account-sidebar-open"); else htmlElement.classList.remove("account-sidebar-open");
        }
        window["FLS"] = true;
        isWebp();
        menuInit();
